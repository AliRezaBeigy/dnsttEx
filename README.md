...
## Changes in this fork (dnsttEx)

This fork adds the following on top of upstream dnstt (after commit ae95dda):
...
- **Client-side SOCKS tunnel mode** — In `-tunnel socks` mode, the **client** runs a SOCKS5 server and sends the destination per stream; the **server** dials the requested target directly. This removes the need for a server-side SOCKS/SSH hop and reduces extra SOCKS handshakes crossing the DNS tunnel. In this fork, both client and server default `-tunnel` to `socks`.
...
<!-- test-results-start -->
**Last run:** 2026-03-06 04:34 · 42 passed, 0 failed, 0 skipped · 74.8s

- **Throughput** (multiple streams at once): 7.1 MB/s over 10 connections (160 KB in 22 ms)

- **Latency** (round-trip time): median 50 ms, 95th %ile 51 ms, 99th %ile 52 ms (n=100)

- **Wire overhead** (DNS encoding): 3.0× — 128 KB payload becomes 378 KB on the wire

- **Reconnect** (after server restart): 59.5 s
